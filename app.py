...
if run_button:
    if cv_file is None or job_file is None:
        ...
        col1.metric(
            "Score global",
            f"{round(total_score, 3)}",
            f"{percentage} %"
        )
        col2.metric("Hard skills", round(scores.get("hard_skills", 0), 3))
        col3.metric("Soft skills", round(scores.get("soft_skills", 0), 3))

        st.progress(float(scores.get("total", 0)))
